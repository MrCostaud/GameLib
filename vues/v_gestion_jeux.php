...
<h1>Ajouter un Nouveau Jeu</h1>
    <form action="index.php?uc=gestion&action=ajouterJeu" method="post">
        <div>
            <label for="nom">Nom du Jeu :</label>
            <input type="text" id="nom" name="nom" required placeholder="Entrez le nom du jeu">
        </div>

        <div>
            <label for="console">Console :</label>
            <select id="console" name="console" required>
                <option value="" disabled selected>Choisissez une console</option>
                <?php foreach ($lesConsoles as $laConsole) : ?>
                    <option value="<?= htmlspecialchars($laConsole['id']) ?>"><?= htmlspecialchars($laConsole['nom_console']) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
